...
import { ExpressP11ParameterTypeResolutionType, ExpressP11ParameterTypeResolver } from "./express-p11-type-utilities";
        ...
        if (
          ...
          if (isComplex_Primary_reference(leftMember) || isProcedure_call_Or_Assigment_stmt_reference(leftMember)) {
            ...
            switch (leftNode.$type) {
              ...
              case "Attribute_id":
                if (
                  !isExplicit_attr(leftNode.$container) &&
                  !isDerived_attr(leftNode.$container) &&
                  !isInverse_attr(leftNode.$container)
                )
                  return EMPTY_SCOPE;
